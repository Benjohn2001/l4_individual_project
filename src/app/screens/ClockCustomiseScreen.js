...
import { getDatabase, update, ref, set, query, get } from "firebase/database";
...
function ClockCustomiseScreen({ route, navigation }) {
  const { locations } = route.params;
  const { membersRef } = route.params;
    ...
    <SafeAreaView className="flex-1 bg-primaryPurple">
      <View className="flex-row justify-between pt-14 mx-10">
        <PressableIcon
          ...
          color="black"
        />
        <Text className="font-bold ml-auto mr-auto text-3xl">Customise</Text>
      </View>
      ...
      >
        <View className=" w-11/12 h-50 items-center py-3 px-3 bg-secondaryPurple rounded-2xl">
          ...
          <Text className="text-center text-gray-500 py-4">
            Select which location to change
          </Text>
          <SelectDropdown
      ...
      >
        <View className=" w-full  items-center py-3 px-3 bg-secondaryPurple rounded-2xl">
          <Text className="text-2xl font-bold text-center  pb-2">
            Change Clock Face
          </Text>
          {newColourEntry ? (
            <>
              ...
              <View className="pt-10 w-11/12 items-center">
                <AppButtonPurple
                  ...
                  onPress={async () => {
                    ...
                    const file = stref(
                      getStorage(),
                      `images/clockFaces/${membersRef}`
                    );
                    await getDownloadURL(file)
                      .then(
                        deleteObject(
                          stref(getStorage(), `images/clockFaces/${membersRef}`)
                        )
                      )
                      .catch((error) => {
          ...
          {newColourEntry || newImageEntry ? (
            ...
            <>
              <Text className="text-center text-gray-500 py-4">
                Choose either a colour or image
              </Text>
              <TwoButtonsSide
                title1="Colour"
                onPress1={() => {
            ...
            <AppButtonPurple
              ...
              onPress={async () => {
                if (colours.includes(newHandColour)) {
                  Alert.alert(
                    "Hand Colour Taken",
                    "Please select a different colour"
                  );
                } else {
